    ...
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full z-[9999] bg-neutral-900 border-t border-neutral-800 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
        <div className="flex gap-3">
          ...
          <button onClick={toggleModal} className="w-[40%] flex items-center justify-center bg-neutral-800 text-white font-bold text-sm uppercase tracking-wide border border-neutral-700 rounded hover:bg-neutral-700 transition">
            Start Quote
          </button>
        </div>
      </div>
      ...
      <div className="hidden md:flex flex-col gap-4 fixed bottom-8 right-8 z-[9999] items-end">
        ...
        <button onClick={toggleModal} className="group relative w-16 h-16 bg-neutral-800 border-2 border-emerald-500 rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-300">
          <span className="text-2xl">☰</span>
        ...
        <div className="fixed inset-0 z-[10000] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="relative w-full max-w-2xl bg-neutral-900 border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-4 bg-neutral-950 border-t border-neutral-800 text-center">
              <button onClick={closeModal} className="text-sm text-gray-500 hover:text-white transition">Close Menu</button>
            </div>
          </div>
        </div>
      )}
